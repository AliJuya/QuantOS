...
from qcore.data.catalog import ResolvedCsvBarDataset
...
class CsvBarReplaySource:
    dataset: ResolvedCsvBarDataset
    ...
    def iter_events(self) -> Iterator[BarCloseEvent]:
        ...
        for path, row in iter_csv_rows(paths=self.dataset.paths, required_columns=required_columns):
            ...
            if last_key is not None and event_key < last_key:
                raise ValueError(
                    f"non-monotonic csv replay ordering detected at {path}: "
