...
from qcore.data.catalog import ResolvedParquetTradeDataset
...
class ParquetTradeReplaySource:
    dataset: ResolvedParquetTradeDataset
    ...
    def iter_events(self) -> Iterator[TradeEvent]:
        ...
        for path, row in iter_parquet_rows(
            ...
            if last_key is not None and event_key < last_key:
                raise ValueError(
                    f"non-monotonic parquet trade replay ordering detected at {path}: "
